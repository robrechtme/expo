...
SSO is available for [Enterprise plan](https://expo.dev/pricing) customers and set up is self-service.

To get started, first prepare your identity provider (IdP) for Expo SSO and gather information by following a configuration guide below.
Once you have done this, the owner of your Organization can enable SSO, as described below.

If you have setup questions or issues, [contact us](https://expo.dev/contact) and we'll help you set up your organization.
...
## Enable SSO
SSO is enabled by creating SSO configuration for the Organization.

<Step label="1">
	Log in as the Organization account owner, select your Organization Account, then go to Organization Settings > Overview.
</Step>
...
<Step label="3">
	Enter the configuration details for your IdP in the form, with the information from setting up your IdP:
  - your client ID
  - your client secret
  - your IdP subdomain/tenant ID, if needed. Check the Issuer field help for what to enter.
</Step>

<Step label="4">
	Click ‘Create SSO Configuration’ and you should see the message that you have successfully created SSO configuration for your account.
	</Step>

<Step label="5">
  The Organization Settings > Overview page will now display an ‘Update SSO configuration’ option.
  - use this option to update the client secret when it has changed.
</Step>
